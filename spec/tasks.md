...
- [ ] 1. Create `qd-datalake-{env}` S3 bucket
...
- [ ] 2. Create `qd-frontend-{env}` S3 bucket and `qd-cdn-{env}` CloudFront distribution
...
- [ ] 3. Create `qd-userpool-{env}` Cognito User Pool with OIDC federation and groups
...
- [ ] 4. Create `qd-api-{env}` API Gateway REST API with Cognito authorizer
...
- [ ] 5. Implement `qd-ingest-api-sonarqube-{env}` and `qd-ingest-api-gitlab-{env}` Lambdas
...
- [ ] 6. Implement `qd-ingest-sftp-fortify-{env}`, `qd-ingest-sftp-trivy-{env}`, `qd-ingest-sftp-tricentis-{env}` Lambdas
...
- [ ] 7. Create `qd-ingest-queue-{env}` SQS queue and `qd-ingest-dlq-{env}` with redrive policy
...
- [ ] 8. Implement `qd-normalize-{tool}-{env}` Lambdas
...
- [ ] 9. Create `qd-db-{env}` RDS MySQL instance and schema migrations
...
- [ ] 10. Implement `qd-gate-eval-{env}` Lambda
...
- [ ] 11. Create `qd-alerts-{project}-{env}` and `qd-ops-alerts-{env}` SNS topics
...
- [ ] 12. Implement `qd-notify-slack-{env}` Lambda
...
- [ ] 13. Create `qd-nightly-pipeline-{env}` Step Functions state machine and `qd-nightly-trigger-{env}` EventBridge rule
...
- [ ] 14. Implement API handler Lambdas `qd-api-{resource}-{env}`
...
- [ ] 15. Configure Secrets Manager entries
...
- [ ] 16. Configure SSM Parameter Store config entries
...
- [ ] 17. Configure CloudWatch Log Groups, alarms, and X-Ray tracing
...
- [ ] 18. Wire IAM role for `qd-ingest-api-*` / `qd-ingest-sftp-*` Lambdas
...
- [ ] 19. Wire IAM role for `qd-normalize-*` Lambdas
...
- [ ] 20. Wire IAM role for `qd-gate-eval` Lambda
  - `rds-db:connect` on `qd-db-{env}`, `sns:Publish` on `qd-alerts-*`
  - _Requirements: 4_
  - _Verify: unit test parses the IAM policy document and asserts only the listed actions/resources are present_

- [ ] 21. Wire IAM role for `qd-notify-slack` Lambda
...
- [ ] 22. Wire IAM role for `qd-api-*` Lambdas and Cognito group authorization
...
- [ ] 23. Wire IAM role for `qd-nightly-pipeline` Step Functions state machine
  - `lambda:InvokeFunction` on all pipeline Lambdas, `states:StartExecution` trigger from EventBridge
  - _Requirements: 1, 4_
  - _Verify: unit test parses the IAM policy document and asserts only the listed actions/resources are present_

- [ ] 24. Wire CI/CD external caller access to `/releases/{id}/gate-check`
...
- [ ] 25. Resolve OED-03 threshold versioning approach before implementing threshold write path
